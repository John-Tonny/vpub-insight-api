  ...
  if (output.data)
  {
    ...
    while(offset < data_bytes.length)
    {
      ...
      if (type == 10) // DEV_FUND_CFWD
      {
        ...
        {  
            var strhash = "";
            for (var i = offset+1; i < data_bytes.length; i++)
    	        strhash = strhash  + String.fromCodePoint(data_bytes[i]);
            offset+=data_bytes.length;
            console.log(strhash);
            transformed.v.push({name:"tracehash",value:strhash});
        } else
        {
          transformed.v.push({name:"error",value:version});
        }
     } else
      {
